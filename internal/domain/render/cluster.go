...
func renderYDBComponent(c *domain.Topology_Component, topo *domain.Topology) *renderpb.Config {
	...
	// A single-node static config with a file-backed pdisk (no /dev block device in
	// a VM/container). Minimal but complete enough for `ydbd server` to bootstrap.
	...
	// Format a file pdisk + start the storage node (ydbd needs its lib dir on the path).
	items = append(items, commandItem("start_storage", strings.Join([]string{
		"install -d /opt/ydb/data",
		"LD_LIBRARY_PATH=/opt/ydb/lib /opt/ydb/bin/ydbd admin bs disk obliterate " + pdisk + " 2>/dev/null || truncate -s 8G " + pdisk,
		"systemd-run --unit=stroppy-ydb-storage --setenv=LD_LIBRARY_PATH=/opt/ydb/lib --collect " +
			"/opt/ydb/bin/ydbd server --yaml-config /etc/ydb/config.yaml --grpc-port 2135 --ic-port 19001 --mon-port 8765 --node 1",
	}, " && "), nil))
