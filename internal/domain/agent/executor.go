...
func (e *Executor) configMySQL(ctx context.Context, cmd Command) error {
	...
	// localHost is needed both inside the rendered my.cnf body (when GR is on)
	// and for placeholder substitution on a user-supplied override.
	...
	if body == "" {
		body = dbconfig.RenderMySQLConf(dbconfig.RenderMySQLConfOpts{
			...
			GroupRepl:     cfg.GroupRepl,
			Options:       cfg.Options,
			TotalMemoryMB: memMB,
		})
	}
	body = dbconfig.SubstituteMySQLPlaceholders(body, cfg.NodeIndex, localHost)

	confPath := "/etc/mysql/my.cnf"
	writeScript := fmt.Sprintf("mkdir -p /etc/mysql && cat > %s << 'MYCNF'\n%sMYCNF", confPath, body)
	if _, err := e.shell(ctx, writeScript); err != nil {
...
func (e *Executor) configPicodata(ctx context.Context, cmd Command) error {
	...
	// Resolve the per-node advertise host the same way as before — explicit
	// AdvertiseHost wins, then $HOSTNAME, then os.Hostname(). Used both to
	// substitute the placeholder and to derive memtx bytes from the body.
...
func (e *Executor) installMonitor(ctx context.Context, cmd Command) error {
	...
	if _, err := e.shell(ctx, "which vector"); err != nil {
		...
		if _, err := e.shell(ctx, vecScript); err != nil {
			...
			e.emitLine(fmt.Sprintf("install vector failed (logs to VictoriaLogs disabled): %v", err))
		}
	}
...
func (e *Executor) configMonitor(ctx context.Context, cmd Command) error {
	...
	if cfg.LogsEndpoint != "" {
		if err := e.startVector(ctx, cfg, machineID); err != nil {
			// Log shipping is non-critical — surface error but don't fail the run.
			e.emitLine(fmt.Sprintf("vector start failed (logs to VictoriaLogs disabled): %v", err))
		}
	}
...
// startVector writes a Vector config that tails journald (and DB-specific log
// files) and pushes JSON-line entries to VictoriaLogs through vmauth. Stream
// fields (run_id, machine_id, role, unit) are baked into each event so the UI
// can pivot logs by run + per-component.
func (e *Executor) startVector(ctx context.Context, cfg MonitorSetupConfig, machineID string) error {
	if _, err := e.shell(ctx, "which vector"); err != nil {
...
func buildVectorConfig(cfg MonitorSetupConfig, machineID string) string {
	...
	// systemd journal — captures stdout/stderr of every systemd-managed
	// service (postgres, mysql, ydbd-*, cockroach, picodata, patroni,
	// haproxy, pgbouncer, vmagent, etc.).
	b.WriteString("sources:\n")
	b.WriteString("  journald:\n")
	b.WriteString("    type: journald\n")
	b.WriteString("    current_boot_only: true\n")

	// File-based DB log tailers for engines that prefer files over journald.
	// Postgres writes to /var/log/postgresql/*.log; MySQL/MariaDB to
	// /var/log/mysql/{error,slow,general}.log.
	...
	// Tag every event with run/machine identifiers so VictoriaLogs queries
...
func (e *Executor) configPatroni(ctx context.Context, cmd Command) error {
	...
	if body == "" {
		body = dbconfig.RenderPatroniConf(dbconfig.RenderPatroniConfOpts{
			...
			PGOptions: cfg.PGOptions,
		})
	}
	body = dbconfig.SubstitutePatroniPlaceholders(body, cfg.Name, cfg.NodeName, cfg.ConnectAddr, cfg.EtcdHosts)

	dataDir := fmt.Sprintf("/var/lib/postgresql/%s/main", pgVersion)

	confPath := "/etc/patroni/patroni.yml"
	writeScript := fmt.Sprintf("mkdir -p /etc/patroni && cat > %s << 'PATRONICFG'\n%sPATRONICFG", confPath, body)
	if _, err := e.shell(ctx, writeScript); err != nil {
...
func (e *Executor) configPgBouncer(ctx context.Context, cmd Command) error {
	...
	body = dbconfig.SubstitutePgBouncerPlaceholders(body, cfg.PGHost)
...
func (e *Executor) configProxySQL(ctx context.Context, cmd Command) error {
	...
	body = dbconfig.SubstituteProxySQLBackends(body, cfg.Backends)

	confPath := "/etc/proxysql.cnf"
	writeScript := fmt.Sprintf("cat > %s << 'PSQLCFG'\n%sPSQLCFG", confPath, body)
	if _, err := e.shell(ctx, writeScript); err != nil {
...
// resolveYDBVersion maps a short version like "25.3" to the full "25.3.1.25".
// If the input is already a full version (e.g. "25.3.1.25"), returns it as-is.
...
func (e *Executor) installYDB(ctx context.Context, cmd Command) error {
	...
	// Map short versions (e.g. "25.3") to full patch versions available at binaries.ydb.tech.
...
func (e *Executor) configYDB(ctx context.Context, cmd Command) error {
	...
	// Memory budget: prefer the topology-spec value passed through; fall back
	// to /proc/meminfo so legacy callers (and recovery from older snapshots)
	// keep working.
	memMB := cfg.MemoryMB
	if memMB <= 0 {
		memMB = getTotalMemoryMB()
