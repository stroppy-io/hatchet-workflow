...
// ProviderSchemas yields the settings schema for one provider, against which a
// tenant's provider config is validated and baked. Get returns derrors.ErrNotFound
// for a provider with no registered settings schema.
type ProviderSchemas interface {
	SettingsSchema(ctx context.Context, provider deployment.Provider) (*schemapb.Schema, error)
}

// ProviderSettingsBaker validates the client-supplied form VALUES (schemapb.Filled)
// against the provider's settings schema and freezes them into a self-contained
// schemapb.Baked. A validation failure must surface as a derrors.Invalid (so the
// handler maps it to InvalidArgument); the engine never trusts the schema ref the
// client embedded — it bakes against the server-owned schema.
type ProviderSettingsBaker interface {
	Bake(ctx context.Context, schema *schemapb.Schema, filled *schemapb.Filled) (*schemapb.Baked, error)
}
...
func (s *TenantSettingsService) UpdateTenantSettings(ctx context.Context, req *api.UpdateTenantSettingsRequest) (*api.UpdateTenantSettingsResponse, error) {
	...
	if err := validateUniqueProviders(req.GetSettings().GetProviders()); err != nil {
		return nil, err
	}
	if _, err := s.requireTenant(ctx, req.GetTenantId()); err != nil {
		return nil, err
...
// keyed by provider). The client sends only the form VALUES (schemapb.Filled); the
// server validates them against the provider's server-owned settings schema and
// bakes the result, then upserts it into the tenant's providers list at most once
// per provider.
func (s *TenantSettingsService) SetTenantProviderSettings(ctx context.Context, req *api.SetTenantProviderSettingsRequest) (*api.SetTenantProviderSettingsResponse, error) {
	c, err := s.caller(ctx)
	if err != nil {
		return nil, err
	}
	...
	schema, err := s.d.Schemas.SettingsSchema(ctx, req.GetProvider())
	if err != nil {
		return nil, utils.MapErr(err)
	}
	baked, err := s.d.Baker.Bake(ctx, schema, req.GetSettings())
	if err != nil {
		return nil, utils.MapErr(err)
	}
	cfg := &deployment.ProviderSettings{
		Provider: req.GetProvider(),
		Settings: baked,
	}

	saved, err := doTxRet(ctx, s, func(ctx context.Context) (*deployment.ProviderSettings, error) {
		existing, err := s.loadOrInit(ctx, req.GetTenantId(), c.GetAccountId())
		if err != nil {
			return nil, err
		}
		existing.Entity = s.mergeEntity(existing.GetEntity(), req.GetTenantId(), c.GetAccountId())
		existing.Providers = upsertProvider(existing.GetProviders(), cfg)
	...
	return &api.SetTenantProviderSettingsResponse{Settings: saved}, nil
}
...
// validateUniqueProviders enforces the "at most one entry per Provider" invariant
// on a settings record's providers list.
func validateUniqueProviders(providers []*deployment.ProviderSettings) error {
	seen := make(map[deployment.Provider]bool, len(providers))
	for _, p := range providers {
		if p.GetProvider() == deployment.Provider_PROVIDER_UNSPECIFIED {
			return status.Error(codes.InvalidArgument, "provider settings entry has unspecified provider")
		}
		if seen[p.GetProvider()] {
			return status.Error(codes.InvalidArgument, "duplicate provider in settings")
		}
		seen[p.GetProvider()] = true
	}
	return nil
}

// upsertProvider replaces the entry for cfg.Provider in place, or appends it,
// keeping at most one entry per provider.
func upsertProvider(list []*deployment.ProviderSettings, cfg *deployment.ProviderSettings) []*deployment.ProviderSettings {
	for i, p := range list {
		if p.GetProvider() == cfg.GetProvider() {
			list[i] = cfg
			return list
		}
	}
