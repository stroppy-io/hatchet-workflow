...
func TestExpandMySQLSingle(t *testing.T) {
	vms, err := Expand("mysql", map[string]any{"topology": "single"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestExpandMySQLPrimaryReplicas(t *testing.T) {
	db := map[string]any{"topology": "primary_replicas", "replica_count": float64(2)}
	vms, err := Expand("mysql", db)
	if err != nil {
		t.Fatal(err)
	}
...
func TestExpandMySQLGroupReplication(t *testing.T) {
	db := map[string]any{"topology": "group_replication", "replica_count": float64(2)}
	vms, err := Expand("mysql", db)
	if err != nil {
		t.Fatal(err)
	}
	if len(vms) != 3 || count(vms, RoleDatabase) != 1 || count(vms, RoleReplica) != 2 {
		t.Fatalf("group_replication: want 1 db + 2 replicas, got %+v", vms)
	}
}

func TestExpandMySQLWithProxy(t *testing.T) {
	db := map[string]any{
