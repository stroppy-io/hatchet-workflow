...
// tuningSection is the my.cnf surface (pure DB). Fields are flat inside one object
// and bucketed for the UI by Group(). Sizes are strings (units / percents / ${var});
// numerics are typed.
func tuningSection() schemapb.FieldDef {
	return schemapb.Object(FieldTuning,
		...
		// 8.0.30+/8.4: dynamic redo log. innodb_log_file_size is the legacy knob.
		sizeField(FieldInnodbRedoLogCapacity, "1G", groupInnoDB, "innodb_redo_log_capacity").
			Desc("MySQL 8.0.30+ (replaces innodb_log_file_size). Size for ~1h of write traffic. "+
				"Ignored/auto on 8.4 when innodb_dedicated_server is ON."),
		sizeField(FieldInnodbLogFileSize, "", groupInnoDB, "innodb_log_file_size").
			Desc("Legacy redo knob (pre-8.0.30). Empty = unset; prefer innodb_redo_log_capacity."),
		...
		schemapb.Int32(FieldInnodbFlushLogAtTrxComm).In(0, 1, 2).Default(1).Group(groupInnoDB).
			...
			Desc("1 = full ACID durability; 2/0 trade durability for throughput (bench-only)."),
		...
		schemapb.Int32(FieldInnodbAutoincLockMode).In(0, 1, 2).Default(2).Group(groupInnoDB).
			...
			Desc("2 (interleaved) is fastest and required with ROW binlog / Group Replication."),
		schemapb.Bool(FieldInnodbDedicatedServer).Default(false).Group(groupInnoDB).
			Title("innodb_dedicated_server").
			Desc("Auto-size buffer pool / redo / flush from system RAM. ON by default on 8.4."),
		schemapb.Bool(FieldInnodbDoublewrite).Default(true).Group(groupInnoDB).
			Title("innodb_doublewrite").Desc("Protects against torn pages; off = bench-only."),
		...
		sizeField(FieldThreadStack, "1M", groupConnections, "thread_stack"),
		...
		schemapb.Int64(FieldBinlogExpireSeconds).Gte(0).Default(604800).Unit("s").
			Group(groupDurability).Title("binlog_expire_logs_seconds").Desc("Default 7 days."),
