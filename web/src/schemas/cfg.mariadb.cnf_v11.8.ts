// GENERATED from schemapb schema cfg.mariadb.cnf@11.8 — do not edit.
// MariaDB 11.8 server configuration rendered to /etc/mysql/mariadb.conf.d/50-server.cnf ([mariadb] section).

/** root */
export interface CfgMariadbCnf11_8 {
  ...
  /** File per table. Store each table in its own .ibd tablespace. Deprecated since 11.0.1 — the shared tablespace is on its way out. */
  ...
  /** Doublewrite buffer. Torn-page protection. 11.x adds `fast`, which skips the doublewrite fsync on already-durable storage. */
  innodb_doublewrite?: "ON" | "OFF" | "fast";
  /** Buffer the redo log. Let the OS page cache buffer redo writes. Part of the set that replaced innodb_flush_method in 11.0. */
  innodb_log_file_buffering?: "ON" | "OFF";
  /** Buffer the data files. Let the OS page cache buffer data-file writes; OFF is the O_DIRECT equivalent. */
  innodb_data_file_buffering?: "ON" | "OFF";
  /** Transaction isolation. Default isolation level; the MySQL-compatible name, available since 11.1. tx_isolation still works but is deprecated. */
  transaction_isolation?: "READ-UNCOMMITTED" | "READ-COMMITTED" | "REPEATABLE-READ" | "SERIALIZABLE";
  /** Snapshot isolation. REPEATABLE READ becomes true snapshot isolation: a write-write conflict raises error 1020 instead of silently reading an older row. 11.8 turned this ON by default; OFF restores the 11.4 behavior. */
  innodb_snapshot_isolation?: "ON" | "OFF";
  /** Temp space per session. Cap on the temporary tables and files one session may occupy on disk; 0 disables the cap. New in 11.5. [MB] */
  max_tmp_session_space_usage?: number | string;
  /** Temp space total. Cap on the temporary disk space of the whole server; 0 disables the cap. New in 11.5. [MB] */
  max_tmp_total_space_usage?: number | string;
  /** Always-log slow time. Queries slower than this are written to the slow log whatever the other log_slow_* filters say. New in 11.7; 0 means the filters decide. [s] */
  log_slow_always_query_time?: number | string;
  /** Replica abort blocking timeout. How long the replica waits for a local query that blocks replication before killing it. New in 11.7; the upstream default of 31536000 s (a year) means never. [s] */
  slave_abort_blocking_timeout?: number | string;
