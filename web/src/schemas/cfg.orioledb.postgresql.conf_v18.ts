// GENERATED from schemapb schema cfg.orioledb.postgresql.conf@18 — do not edit.
// postgresql.conf for the OrioleDB build of PostgreSQL 18: the stock 18 parameters plus the orioledb.* settings.

/** root */
export interface CfgOrioledbPostgresqlConf18 {
  ...
  /** Effective I/O concurrency. Number of concurrent I/O requests issued for the new asynchronous I/O subsystem. Upstream default raised to 16 in PostgreSQL 18. */
  ...
  /** I/O method. Backend for asynchronous I/O. New in PostgreSQL 18; `io_uring` requires a build with liburing. */
  io_method?: "worker" | "io_uring" | "sync";
  /** I/O workers. Number of I/O worker processes; only used when io_method = worker. New in PostgreSQL 18. */
  io_workers?: number | string;
  ...
  /** Autovacuum worker slots. Worker slots reserved at startup, the ceiling autovacuum_max_workers can be raised to without a restart. New in PostgreSQL 18. */
  autovacuum_worker_slots?: number | string;
  ...
  /** Vacuum max threshold. Upper bound on the tuple count computed from threshold + scale factor, so huge tables are still vacuumed. -1 disables the cap. New in PostgreSQL 18. */
  autovacuum_vacuum_max_threshold?: number | string;
  ...
  /** Log connections. Connection aspects to log, comma-separated: receipt, authentication, authorization, setup_durations, or all. Empty disables. Became a string in PostgreSQL 18 (a boolean before). */
