...
import { instanceIdeUrl, orgIdeUrl } from "@/services/ide";
import { EmbeddedIde } from "@/components/ide/EmbeddedIde";

// CatalogEntryEditor only ever deals in providers/workflows (never
// "recipes" — that tab exists on IdeEntryTab for RecipeEditor.tsx's own
// use), so this local alias keeps instanceIdeUrl's narrower
// Exclude<IdeEntryTab, "recipes"> param happy without importing IdeEntryTab
// itself here.
type CatalogIdeTab = "providers" | "workflows";
...
// EmbeddedIde (the ticket -> iframe handshake) now lives in
// @/components/ide/EmbeddedIde — extracted so RecipeEditor.tsx's own IDE
// branch reuses the exact same component rather than a second copy. See
// that file for the full sequence doc (ticket mint, same-origin iframe,
// SameSite=Lax cookie, the ~15s first-open EnsureRunning latency).
...
export function CatalogEntryEditor({ scope }: CatalogEntryEditorProps) {
  ...
  const tab: CatalogIdeTab = kindParam === "workflows" ? "workflows" : "providers";
