// Database Preset authoring — CREATE (/presets/database/new) and EDIT
// (/presets/database/:id/edit).
//
// Built against cloud.v1.api.DatabasePresetService: the form edits one
// cloud.v1.models.DatabasePresetRecord = Entity (name/description/tags) + the
// typed cloud.v1.domain.Database (engine Kind + per-engine Params). It REUSES
// the wizard's database editor (EngineParamsForm + EngineVersionSelect) and the
// same domain.Database.Validate mirror (validateDatabase) so the create/edit
// surface is identical to the New Run Database step.
//
//   Create → PresetProvider.createDatabasePreset(slug, input)  → CreateDatabasePreset
//   Edit   → PresetProvider.getDatabasePreset(slug, id) to load,
//            then PresetProvider.updateDatabasePreset(slug, id, input) → UpdateDatabasePreset
//
...
// preset's detail page (/presets/database/:id).
...
export function DatabasePresetForm() {
  ...
  // Validation mirrors the backend (domain.Database.Validate). Save is blocked
  // while any error-severity issue (or an empty name) remains.
  const dbErrors = useMemo(() => validateDatabase(db), [db]);
  const blocking = dbErrors.filter((e) => e.severity === "error");
  ...
  const onSave = useCallback(async () => {
    ...
    setSaving(true);
    setSaveError(null);
    const input: DatabasePresetInput = { name: name.trim(), description: description.trim(), tags, database: db };
    try {
      const provider = getPresetProvider();
      if (isEdit && id) {
        await provider.updateDatabasePreset(slug, id, input);
        navigate(`/presets/database/${id}`);
      } else {
        const newId = await provider.createDatabasePreset(slug, input);
  ...
  }, [slug, isEdit, id, name, description, tags, db, navigate]);
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center justify-between border-b border-zinc-800/80 px-5 py-3">
        <div className="flex items-center gap-3 min-w-0">
          <button
            type="button"
            onClick={() => navigate(isEdit && id ? `/presets/database/${id}` : "/presets/database")}
          ...
          <div className="min-w-0">
            <div className="text-[10px] font-mono uppercase tracking-wider text-zinc-600">
              {isEdit ? "Edit database preset" : "New database preset"}
            </div>
            <h1 className="truncate text-base font-semibold tracking-tight text-foreground">
              {name || (isEdit ? "Untitled preset" : "New database preset")}
            </h1>
          </div>
        </div>
