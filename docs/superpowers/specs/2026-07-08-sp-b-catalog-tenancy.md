...
> - **Дефолт-связь instance→org = живая ссылка + fork-on-edit.** org LINKED на instance (видит апдейты) до первой правки; правка авто-форкает (FORKED). `SeedOrgCatalog` линкует (не копирует), `ForkEntry`/`BundleStore.Fork` срабатывает при первом редактировании. Q1 ниже разрешён в пользу «живой ссылки».
> - **provider.use = пиннинг версии** (`yandex@1.2`), резолв по org-каталогу по точной версии (воспроизводимость Run + Baked-идентичность SP-E). Версия обязательна в `provider.use`.
